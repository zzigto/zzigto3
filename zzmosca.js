...
class zzmosca
{
    ...
    /*saveData(topic, jmessage) 
	{
		var items=GLOBAL_MOSCA.splitTopic(topic);
		console.log("saveData:", topic, items, jmessage);
		if (items.root && items.device)
		{
			...
			console.log("T:", jmessage.T, "H:", H, "D:", D);
			...
			if (!fs.existsSync(F))
			{
				...
				console.log("passo1", JSON.stringify(jdata, null, 2));
				...
				{	
					...
					if (jdata[H])
					{
						...
						for (var prop in jmessage) 
						{
							try
							{
								if (jmessage.value[prop].TN && jmessage.value[prop].N<jdata[H][prop].N)
								{
									jdata[H][prop].N=jmessage.value[prop].N;
									jdata[H][prop].TN=jmessage.value[prop].TN;
								}
								//else
								//	console.log("passo.a.3.1 skip N");
							}
							catch(e)
							{
							}
							...
							try
							{
								if (jmessage.value[prop].TM && jmessage.value[prop].M>jdata[H][prop].M)
								{
									jdata[H][prop].N=jmessage.value[prop].M;
									jdata[H][prop].TN=jmessage.value[prop].TM;
								}
								//else
								//	console.log("passo.a.3.2 skip M");
							}
							catch(e)
							{
							}
